...
public class BeamIOSourceRel extends TableScan implements BeamRelNode {
  ...
  public PCollection<BeamSqlRecord> buildBeamPipeline(PCollectionTuple inputPCollections
    ...
    TupleTag<BeamSqlRecord> sourceTupleTag = new TupleTag<>(sourceName);
    if (inputPCollections.has(sourceTupleTag)) {
      //choose PCollection from input PCollectionTuple if exists there.
      PCollection<BeamSqlRecord> sourceStream = inputPCollections
          .get(new TupleTag<BeamSqlRecord>(sourceName));
      return sourceStream;
    } else {
